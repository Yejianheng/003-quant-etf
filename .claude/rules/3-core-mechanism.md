...
| `execution_lag=1` | T-1 日收盘价算信号 → **T 日开盘价成交** → T 日收盘估值 | **现实可行**（盘后跑信号，次日开盘执行） | ~1.18 | ~191% | -8.9% |

**v211 关键修正**：`execution_lag=1` 执行价从收盘改为开盘，执行顺序改为"先执行(open)→再估值(close)→再信号"。旧逻辑用 T 日收盘价执行 T-1 信号，已清仓持仓仍吃全天跌幅；新逻辑开盘即换仓，仅承担真实的隔夜跳空损失。

**scale_factor 安全网（v212）**：`total_at_open < exec_alloc["total_capital"]` 时按 `total_at_open / total_capital` 等比缩放目标金额，杜绝隔夜大幅跳空导致的负现金（合成 10% 跳空测试通过；日常数据因 vol_scaling 现金缓冲充裕，极少触发）。
...
```
图表 T 日持仓 = T-1 信号，T 日 Δ% = (NAV_T/NAV_{T-1})-1，含隔夜跳空+日内涨跌。

**历史**：早期 Look-Ahead Bias 验证时 T+1 数据因现金泄漏 bug（repo_cash 取值错误）被压至 Sharpe 0.02。修复后 T+1 恢复至 1.02（commit `17aea9e` → `f88bcd6`）。v211 开盘价执行后 T+1 升至 ~1.18（与 T+0 差距大幅缩小，因新逻辑开盘即捕获当日收益）。
